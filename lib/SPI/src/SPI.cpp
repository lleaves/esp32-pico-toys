...
SPIClass::SPIClass(uint8_t spi_bus)
    : _spi_num(spi_bus), _spi(NULL), _use_hw_ss(false), _sck(-1), _miso(-1), _mosi(-1), _ss(-1), _div(0),
      _freq(40000000), _inTransaction(false)
{}

void SPIClass::begin(int8_t sck, int8_t miso, int8_t mosi, int8_t ss, uint32_t frequency)
{
    if (_spi)
    {
        return;
    ...
    if (!_div)
    {
        _div = spiFrequencyToClockDiv(frequency);
    }
