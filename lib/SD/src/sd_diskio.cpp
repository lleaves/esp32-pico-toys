...
char sdCommand(uint8_t pdrv, char cmd, unsigned int arg, unsigned int* resp)
{
    ...
    if (token == 0xFF)
    {
        // LV_LOG_ERROR("Card Failed! cmd: 0x%02x", cmd);
...
bool sdReadSector(uint8_t pdrv, char* buffer, unsigned long long sector)
{
    for (int f = 0; f < 3; f++)
    {
        ...
        if (!sdCommand(pdrv, READ_BLOCK_SINGLE, (s_cards[pdrv]->type == CARD_SDHC) ? sector : sector << 9,
                       NULL))
        {
...
bool sdReadSectors(uint8_t pdrv, char* buffer, unsigned long long sector, int count)
{
    for (int f = 0; f < 3;)
    {
        ...
        if (!sdCommand(pdrv, READ_BLOCK_MULTIPLE, (s_cards[pdrv]->type == CARD_SDHC) ? sector : sector << 9,
                       NULL))
        {
            do
            {
...
bool sdWriteSector(uint8_t pdrv, const char* buffer, unsigned long long sector)
{
    for (int f = 0; f < 3; f++)
    {
        ...
        if (!sdCommand(pdrv, WRITE_BLOCK_SINGLE, (s_cards[pdrv]->type == CARD_SDHC) ? sector : sector << 9,
                       NULL))
        {
...
bool sdWriteSectors(uint8_t pdrv, const char* buffer, unsigned long long sector, int count)
{
    ...
    for (int f = 0; f < 3;)
    {
        ...
        if (!sdCommand(pdrv, WRITE_BLOCK_MULTIPLE,
                       (card->type == CARD_SDHC) ? currentSector : currentSector << 9, NULL))
        {
            do
            {
...
bool sdcard_mount(uint8_t pdrv, const char* path, uint8_t max_files, bool format_if_empty)
{
    ...
    {
        // LV_LOG_ERROR("esp_vfs_fat_register failed 0x(%x): SD is registered.", err);
        return false;
    } else if (err != ESP_OK)
    {
        // LV_LOG_ERROR("esp_vfs_fat_register failed 0x(%x)", err);
        return false;
    }
    ...
    if (res != FR_OK)
    {
        // LV_LOG_ERROR("f_mount failed: %s", fferr2str[res]);
        if (res == 13 && format_if_empty)
        {
            ...
            if (res != FR_OK)
            {
                // LV_LOG_ERROR("f_mkfs failed: %s", fferr2str[res]);
